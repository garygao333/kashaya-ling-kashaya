...
import data from '../static/result_all_words.json';
...
const orderAudio = (paths) => {
  const allSpeakers = ['HJ', 'EP', 'IJ', 'GJ', 'VC', 'ML', 'AS', 'OP', 'IA', 'FD'];
  const newArr = [];
  //  speaker
...
function descendingComparator(a, b, orderBy) {
  function zip() { // see https://stackoverflow.com/questions/4856717/javascript-equivalent-of-pythons-zip-function
  ...
  if (orderBy === 'english') {
    ...
    const letterOrder = [undefined, 'a', 'á', 'b', 'c', 'd', 'e', 'é', 'f', 'h', 'i', 'í', 'k', 'l', 'm', 'n', 'o', 'ó', 'p', 'q', 'r', 's', 'š', 't', 'ṭ', 'u', 'ú', 'w', 'y', 'ʔ', 'ʰ', 'ʼ', '·', ' ']
    for (let [x, y] of zip([...a[orderBy]], [...b[orderBy]])) {
      if (letterOrder.indexOf(x) < letterOrder.indexOf(y)) {
        return 1;
      } else if (letterOrder.indexOf(x) > letterOrder.indexOf(y)) {
        return -1
      } else {
        continue;
      }
 ...
 * -------AllList Component--------
 * --------------------------------*/
export default function WordsAll() {
  ...
  const [lastSearchEvent, setLastSearchEvent] = useState({target: {value: ''}});
  ...
  const handleOrderByChange = () => {
    ...
    delaySearch(lastSearchEvent, rowsTemp, orderBy === 'english' ? 'kashaya' : 'english');
  };
  ...
  const delaySearch = useCallback(
    debounce((event, rowsTemp2, orderBy) => {
      ...
      if (event.target.value === '') {
        ...
        const newRows = rowsNewTemp.filter(
          (row) =>
          orderBy === 'english' ? row.english.toLowerCase().includes(query) :
          orderBy === 'kashaya' ? kashayaQueriesArr.some((q) => row.kashaya.includes(q)) :
          false
        );
        setRows(newRows);
      }
  ...
  const onChangeSearchInput = (event) => {
    event.persist();
    delaySearch(event, rowsTemp, orderBy);
  };
  // eslint-disable-next-line
